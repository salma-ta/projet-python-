...
from classes import Client, Salle, Reservation
...
class TestSalle(unittest.TestCase):
    def test_salle_standard(self):
        salle=Salle(nom="Salle A",type_salle="Standard",capacite=4)
    ...
    def test_salle_conference(self):
        salle=Salle(nom="Salle B",type_salle="Conference",capacite=10)
        self.assertEqual(salle.nom, "Salle B")
        self.assertEqual(salle.type_salle, "Conference")
        self.assertEqual(salle.capacite, 10)
    def test_salle_informatique(self):
        salle=Salle(nom="Salle C",type_salle="Informatique",capacite=4)
        self.assertEqual(salle.nom, "Salle C")
...
class TestReservation(unittest.TestCase):
    def test_reservation_duree_minimale(self):
        ...
        with self.assertRaises(ValueError):
            reservation=Reservation(client_id="123", nom_salle="A", debut=debut_str, fin=fin_str)
